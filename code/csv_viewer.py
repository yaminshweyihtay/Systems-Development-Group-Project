...
class CsvViewer(tk.Frame):
    def __init__(self, container):
        ...
        self.csv_parameters = []
        self.hidden_columns = []
        self.button_bar = ButtonBar(self, None, self.toggle_column, self.get_hidden_columns)
    ...
    def display_csv_data(self, csv_viewer, status_label):
        try:
            with open(self.file_path, 'r', newline='', encoding='utf-8') as file:
                ...
                self.display_button_bar(csv_viewer)
    ...
    def display_button_bar(self, csv_viewer):
        # if button bar is displayed remove it (fixes bug where multiple filter buttons can be displayed at once)
        if self.button_bar.winfo_ismapped():
            self.button_bar.pack_forget()
        self.button_bar = ButtonBar(self, csv_viewer, self.toggle_column, self.get_hidden_columns)
        self.button_bar.pack(padx=40, pady=15, side=TOP, anchor=NW)
